import Link from "next/link";

const NextAllTab = () => {
  return (
    <div className="table__wrap">
      <div className="table__items b__bottom">
        <div className="t__items">
          <div className="t__items__left t__items__left__nextogo">
            <div className="t__items__icon">
              <i className="icon-tennis"></i>
            </div>
            <div className="content">
              <h6>Maximilian Neuchrist AUT</h6>
              <span className="text">Timofey Skatov KAZ</span>
            </div>
          </div>
        </div>
        <div className="mart__point__two mart__pint__nextgo">
          <div className="mart__point__left">
            <Link href="" className="point__box">
              <span className="point__1">1</span>
              <span>2.95</span>
            </Link>
            <Link href="" className="point__box">
              <span className="point__1">2</span>
              <span>1.35</span>
            </Link>
          </div>
          <div className="mart__point__right">
            <Link href="#min" className="point__box-text point__box__nextto">
              <span> Starting in 42 min</span>
              <span className="icons">
                <i className="fas fa-angle-right"></i>
              </span>
            </Link>
          </div>
        </div>
      </div>
      <div className="table__items b__bottom">
        <div className="t__items">
          <div className="t__items__left t__items__left__nextogo">
            <div className="t__items__icon">
              <i className="icon-tennis"></i>
            </div>
            <div className="content">
              <h6>Nuno Borges POR</h6>
              <span className="text">Pedro Sousa POR</span>
            </div>
          </div>
        </div>
        <div className="mart__point__two mart__pint__nextgo">
          <div className="mart__point__left">
            <Link href="" className="point__box">
              <span className="point__1">1</span>
              <span>2.00</span>
            </Link>
            <Link href="" className="point__box">
              <span className="point__1">2</span>
              <span>1.72</span>
            </Link>
          </div>
          <div className="mart__point__right">
            <Link href="#min" className="point__box-text point__box__nextto">
              <span> Starting in 20 min</span>
              <span className="icons">
                <i className="fas fa-angle-right"></i>
              </span>
            </Link>
          </div>
        </div>
      </div>
      <div className="table__items b__bottom">
        <div className="t__items">
          <div className="t__items__left t__items__left__nextogo">
            <div className="t__items__icon">
              <i className="icon-tennis"></i>
            </div>
            <div className="content">
              <h6>Cristina Bucsa ESP</h6>
              <span className="text">Kateryna Baindl UKR</span>
            </div>
          </div>
        </div>
        <div className="mart__point__two mart__pint__nextgo">
          <div className="mart__point__left">
            <Link href="" className="point__box">
              <span className="point__1">1</span>
              <span>2.00</span>
            </Link>
            <Link href="" className="point__box">
              <span className="point__1">2</span>
              <span>1.20</span>
            </Link>
          </div>
          <div className="mart__point__right">
            <Link href="#min" className="point__box-text point__box__nextto">
              <span className="text-base"> Starting in 60 min</span>
              <span className="icons">
                <i className="fas fa-angle-right text-base"></i>
              </span>
            </Link>
          </div>
        </div>
      </div>
      <div className="table__items b__bottom">
        <div className="t__items">
          <div className="t__items__left t__items__left__nextogo">
            <div className="t__items__icon">
              <i className="icon-tennis"></i>
            </div>
            <div className="content">
              <h6>Canada</h6>
              <span className="text">Morocco</span>
            </div>
          </div>
        </div>
        <div className="mart__point__two mart__pint__nextgo">
          <div className="mart__point__left">
            <Link href="" className="point__box">
              <span className="point__1">1</span>
              <span>5.23</span>
            </Link>
            <Link href="" className="point__box">
              <span className="point__1">2</span>
              <span>4.53</span>
            </Link>
          </div>
          <div className="mart__point__right">
            <Link href="#min" className="point__box-text point__box__nextto">
              <span className="text-base"> Starting in 40 min</span>
              <span className="icons">
                <i className="fas fa-angle-right text-base"></i>
              </span>
            </Link>
          </div>
        </div>
      </div>
      <div className="table__items b__bottom">
        <div className="t__items">
          <div className="t__items__left t__items__left__nextogo">
            <div className="t__items__icon">
              <i className="icon-tennis"></i>
            </div>
            <div className="content">
              <h6>Croatia</h6>
              <span className="text">Belgium</span>
            </div>
          </div>
        </div>
        <div className="mart__point__two mart__pint__nextgo">
          <div className="mart__point__left">
            <Link href="" className="point__box">
              <span className="point__1">1</span>
              <span>4.23</span>
            </Link>
            <Link href="" className="point__box">
              <span className="point__1">2</span>
              <span>3.53</span>
            </Link>
          </div>
          <div className="mart__point__right">
            <Link href="#min" className="point__box-text point__box__nextto">
              <span> Today / 21:00</span>
              <span className="icons">
                <i className="fas fa-angle-right"></i>
              </span>
            </Link>
          </div>
        </div>
      </div>
      <div className="table__items b__bottom">
        <div className="t__items">
          <div className="t__items__left t__items__left__nextogo">
            <div className="t__items__icon">
              <i className="icon-tennis"></i>
            </div>
            <div className="content">
              <h6>Canada</h6>
              <span className="text">Belgium</span>
            </div>
          </div>
        </div>
        <div className="mart__point__two mart__pint__nextgo">
          <div className="mart__point__left">
            <Link href="" className="point__box">
              <span className="point__1">1</span>
              <span>2.22</span>
            </Link>
            <Link href="" className="point__box">
              <span className="point__1">2</span>
              <span>3.53</span>
            </Link>
            <Link href="" className="point__box">
              <span className="point__1">2</span>
              <span>3.53</span>
            </Link>
          </div>
          <div className="mart__point__right">
            <Link href="#min" className="point__box-text point__box__nextto">
              <span className="text-base"> Today / 21:00</span>
              <span className="icons">
                <i className="fas fa-angle-right text-base"></i>
              </span>
            </Link>
          </div>
        </div>
      </div>
      <div className="table__items b__bottom">
        <div className="t__items">
          <div className="t__items__left t__items__left__nextogo">
            <div className="t__items__icon">
              <i className="icon-tennis"></i>
            </div>
            <div className="content">
              <h6>Canada</h6>
              <span className="text">Belgium</span>
            </div>
          </div>
        </div>
        <div className="mart__point__two mart__pint__nextgo">
          <div className="mart__point__left">
            <Link href="" className="point__boxbet">
              <span>Bet now</span>
            </Link>
          </div>
          <div className="mart__point__right">
            <Link href="#min" className="point__box-text point__box__nextto">
              <span className="text-base"> Today / 21:00</span>
              <span className="icons">
                <i className="fas fa-angle-right text-base"></i>
              </span>
            </Link>
          </div>
        </div>
      </div>
      <div className="table__footer table__footer__nextgo">
        <Link href="#show" className="lobby">
          <span>Show more</span>
          <span className="icons">
            <i className="fa-solid fa-chevron-down"></i>
          </span>
        </Link>
      </div>
    </div>
  );
};

export default NextAllTab;
